...
void CommandDelete(Instance *instance, EsElement *, EsCommand *) {
	...
	instance->del.index = index;
			FolderEntry *entry = instance->listContents[instance->del.index].entry;
			String name = StringAllocateAndFormat("%z", entry->GetName());
			BlockingTaskQueue(instance, {
				.string = name,
				.cDescription = interfaceString_FileManagerDeleteTask,

				.callback = [] (Instance *instance, Task *task) {
						task->result = instance->folder->itemHandler->deleteItem(instance->folder, task->string);
				},
				...
				.then = [] (Instance *instance, Task *task) {
					if (instance->closed) {
						...
						InstanceReportError(instance, ERROR_DELETE_ITEM, task->result);
					} else {
						Folder *folder = instance->folder;

						size_t PathBytes;
						char *path = EsStringAllocateAndFormat(&PathBytes, "%s%s", STRFMT(instance->folder->path), STRFMT(task->string));

						EsDirectoryChild information = {};
						EsPathQueryInformation(path, PathBytes, &information);
						EsMutexAcquire(&folder->modifyEntriesMutex);
						EsAssert(folder->doneInitialEnumeration);
						FolderRemoveEntryAndUpdateInstances(folder, STRING(task->string));
						EsMutexRelease(&folder->modifyEntriesMutex);
...
void InstanceRegisterCommands(Instance *instance) {
	...
	EsCommandRegister(&instance->commandDelete, instance, INTERFACE_STRING(FileManagerDeleteAction), CommandDelete, stableCommandID++, "F3");
