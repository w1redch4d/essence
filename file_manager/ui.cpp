...
bool InstanceLoadFolder(Instance *instance, String path /* takes ownership */, int historyMode) {
	...
	task.then = [] (Instance *instance, Task *task) {
		...
		EsCommandSetEnabled(&instance->commandDelete, folder->itemHandler->deleteItem && !folder->readOnly);
...
#define COLUMN_SIZE (2)
		...
		EsMenuAddCommand(menu, ES_FLAGS_DEFAULT, INTERFACE_STRING(FileManagerDeleteAction), &instance->commandDelete);
		EsMenuShow(menu);
